...
def buildCityPoints(fName, scaling):
    nameList, featureList = readCityData(fName)
    points = []
    for i in range(len(nameList)):
        point = City(nameList[i], pylab.array(featureList[i]))
...
def test0(numClusters = 2, scaling = 'identity', printSteps = False,
          printHistory = True):
    points = buildCityPoints('cityTemps.txt', scaling)
    cS = ClusterSet(City)
    for p in points:
...
def test1(k = 2, cutoff = 0.0001, numTrials = 1, printSteps = False,
          printHistory = False):
    points = buildCityPoints('cityTemps.txt', '1/max')
    if printSteps:
        print 'Points:'
        for p in points:
    ...
    while numClusterings < numTrials:
        clusters, maxDiameter = kmeans(points, k, cutoff, City)
